...
if __name__ == "__main__":
    ...
    collections = readJSON(filename)
    filename = input("Please enter input filename: ")
    inputFile = readJSON(filename)
    ...
    inputNames = ["" for _ in range(3)]
    abc = ["a", "b", "c"]
    for i in range(3):
        inputNames[i] = input("Please enter the name of the input model for input " + str(abc[i]) + ": ")
    ...
    models = ["" for _ in range(numberGates)]
    for i in range(numberGates):
        modelName = input("Please enter the name of Gate " + str(i) + "'s model in netlist: ")
        # print(modelName) ###############################################################################
    ...
    for count,gate in enumerate(newGates):
        ...
        if improvement[count][0] > 0:
            print('Stronger promoter by a factor of ', round(improvement[count][0],4))
        elif improvement[count][0] < 0: 
            print('Weaker promoter by a factor of ', round(abs(improvement[count][0]),4))
    
        if improvement[count][1] != 0:
            print('Stretch by a factor of ', round(improvement[count][1],4))
  
        if improvement[count][2] > 0: 
            print('Stronger RBS by a factor of ', round(improvement[count][2],4))
        elif improvement[count][2] < 0:
            print('Weaker RBS by a factor of ', round(abs(improvement[count][2]),4))
  
        if improvement[count][3] > 0: 
            print('Increase slope by a factor of ', round(improvement[count][3],4))
        elif improvement[count][3] < 0:
            print('Decrease slope by a factor of ', round(abs(improvement[count][3]),4))
        print("\n")
